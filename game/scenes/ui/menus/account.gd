...
@onready var display_name_input = $Control2/VBoxContainer/DisplayNameInput
@onready var password_input     = $Control2/VBoxContainer/PasswordInput
@onready var status_label       = $Control2/VBoxContainer/StatusLabel
@onready var http_request       = $HTTPRequest
@onready var save_button        = $Control2/SaveButton
@onready var pfp_rect           = $Control/PFP
@onready var file_dialog        = $FileDialog
@onready var upload_request     = $UploadRequest
@onready var change_pfp_button  = $Control/ChangePFPButton
...
func _ready():
	...
	$Control/UsernameDisplay.text = GameManager.user_data.username
	$Control/UsernameDisplay.editable = false

	# Signal connections for HTTPRequest and UploadRequest are defined in the .tscn file

	# Button press shrink animation (visual feedback, not game logic)
	_connect_press_anim(save_button)
	_connect_press_anim(change_pfp_button)
	_connect_press_anim(back_button)
...
func _on_change_pfp_button_pressed():
	...
	file_dialog.show()
	status_label.text = "Select an image file..."
	status_label.add_theme_color_override("font_color", Color(1, 0.9, 0.4, 1))
...
func _on_save_button_pressed():
	...
	# Auto-retry timeout after 10 seconds
	get_tree().create_timer(10.0).timeout.connect(func():
		if save_button.disabled:
			save_button.disabled = false
			status_label.text = "Request timed out"
	)
