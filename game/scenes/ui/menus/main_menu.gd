...
# ── Constants ────────────────────────────────────────────────────────────────
const SCENE_CUSTOM_ROOM = "res://scenes/ui/lobby/custom_room.tscn"
const SCENE_LEADERBOARD = "res://scenes/ui/social/leaderboard.tscn"
const SCENE_LOGIN       = "res://scenes/ui/menus/login_scene.tscn"
...
@onready var friends_button          = $Settings/FriendsButton
@onready var friends_dimmer          = $FriendsDimmer
@onready var friends_panel           = $FriendsPanel
@onready var friends_list            = $FriendsPanel/VBoxContainer/ScrollContainer/FriendsList
@onready var friend_search_input     = $FriendsPanel/VBoxContainer/SearchHBox/SearchInput
@onready var friend_status_label     = $FriendsPanel/VBoxContainer/StatusLabel
@onready var req_unread_label        = $FriendsPanel/VBoxContainer/SearchHBox/ReqBtn/ReqUnreadLabel
@onready var intro_anim_player       = $AnimationPlayer
@onready var settings_anim_player    = $Settings/AnimationPlayer
@onready var close_button           = $Settings/CloseButton
@onready var account_button         = $Settings/AccountButton
@onready var history_button         = $Settings/HistoryButton
@onready var credits_button         = $Settings/Credits
@onready var feedback_button        = $Settings/Feedback
@onready var logout_button          = $Settings/LogoutButton
...
var bot_timeout_seconds: float = 0.0  # Random timeout for bot spawning (30-50s)
var bot_fallback_active: bool = false  # Whether bot fallback is enabled
...
func _ready():
	...
	# Signal connections are defined in the .tscn file
	_setup_chat()
...
func _setup_chat():
	chat_box.room_id = "global"
	# Signal connection for friends_updated is defined in the .tscn file
...
func _process(delta):
	...
	if is_matchmaking:
		...
		# Check for bot fallback timeout
		if bot_fallback_active and elapsed >= bot_timeout_seconds:
			_spawn_bot_opponent()
			return
...
func _on_play_online_pressed():
	...
	# Enable bot fallback - will spawn bot if no human found within 30-50s
	_enable_bot_fallback()
...
func _on_settings_pressed():
	...
	$Settings.show()
	settings_anim_player.play("slide_in")
...
var friend_entry_scene = preload("res://scenes/ui/social/friend_entry.tscn")
...
func _on_history_pressed():
	...
	await settings_anim_player.animation_finished
	$Settings.hide()
	get_tree().change_scene_to_file("res://scenes/ui/social/history.tscn")
...
func _on_account_pressed():
	...
	await settings_anim_player.animation_finished
	$Settings.hide()
	get_tree().change_scene_to_file("res://scenes/ui/menus/account.tscn")
...
func _on_feedback_pressed():
	...
	await settings_anim_player.animation_finished
	$Settings.hide()
	get_tree().change_scene_to_file("res://scenes/ui/menus/feedback.tscn")
...
func _on_req_btn_pressed():
	...
	var btn = friends_panel.get_node("VBoxContainer/SearchHBox/ReqBtn")
	if showing_requests:
...
func _create_friend_entry(data: Dictionary):
	...
	var name_label = entry.get_node("InfoVBox/NameLabel")
	var status_label = entry.get_node("InfoVBox/StatusLabel")
	var status_dot = entry.get_node("AvatarContainer/StatusDot")
	var avatar_icon = entry.get_node("AvatarContainer/AvatarIcon")
	...
	# Unread badge
	var unread_label = entry.get_node_or_null("AvatarContainer/UnreadLabel")
	if unread_label:
...
func _spawn_bot_opponent():
	"""Spawn a bot opponent after matchmaking timeout"""
	if not is_matchmaking:
		return
	
	print("[Matchmaking] Bot timeout reached (%.1fs) - spawning AI opponent" % bot_timeout_seconds)
	
	# Stop matchmaking
	is_matchmaking = false
	bot_fallback_active = false
	
	# Create bot opponent
	var bot_data = BotManager.create_bot_opponent()
	
	# Set up game state for bot match
	GameManager.is_host = true
	GameManager.is_solo = true  # Treat as solo but with bot opponent
	GameManager.is_matchmaking = false
	GameManager.current_room = ""
	
	# Set opponent info from bot
	GameManager.opponent_character = bot_data.character
	GameManager.opponent_passive = bot_data.passive
	GameManager.opponent_name = bot_data.name
	
	# Show "Opponent found!" message briefly
	matchmaking_label.text = "Opponent found!"
	matchmaking_time_label.hide()
	
	# Start the bot match
	BotManager.start_bot_match()
	
	# Transition to game scene after a brief delay
	await get_tree().create_timer(1.0).timeout
	_transition_to_room(SCENE_CUSTOM_ROOM)

func _enable_bot_fallback():
	"""Enable bot fallback during matchmaking"""
	bot_fallback_active = true
	# Random timeout between 30-50 seconds
	bot_timeout_seconds = randf_range(30.0, 50.0)
	print("[Matchmaking] Bot fallback enabled - will spawn bot after %.1f seconds if no human found" % bot_timeout_seconds)
