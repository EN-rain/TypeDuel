...
# CP (Combo Points) - DEPRECATED (Moved to Mana)
var player_cp:    int = 0
var opponent_cp:  int = 0

# Mana (Current Resource)
...
func resolve_round(
	...
	action_log.append_array(status_result.log)
	...
	match HPManager.player_passive:
		"Overdrive": # Zephon
			if wpm > 80:
				...
				action_log.append("[Passive: Overdrive] WPM > 80, gained 1 Mana!")
		"Bloodlust": # Riven
			if won:
				player_win_streak += 1
			else:
			...
			if player_win_streak >= 2:
				action_log.append("[Passive: Bloodlust] 2 wins! Paused 3HP self-damage.")
			else:
				player_hp_delta -= 3
				action_log.append("[Passive: Bloodlust] Took 3 self-damage.")
		"Survivor": # Caelum
			if player_hp < HPManager.player_max_hp * 0.4:
				player_damage += 3
				action_log.append("[Passive: Survivor] Low HP, +3 DMG!")
		"Equilibrium": # Nyxara
			player_damage *= 1.1 # simplified for odd round
			action_log.append("[Passive: Equilibrium] +10% DMG!")
		"Tactician": # Valdris
			if won:
				player_mana = min(10, player_mana + 1)
				action_log.append("[Passive: Tactician] Won round, +1 Mana!")
		"Grace": # Liora
			if accuracy > 80.0:
				player_hp_delta += 3
				action_log.append("[Passive: Grace] Accuracy > 80%, Healed 3HP!")
		"Vengeance": # Malachar
			if opp_damage > 0: # simplifed
				HPManager.player_base_dmg += 2
				action_log.append("[Passive: Vengeance] Took damage, gained +2 permanent DMG!")
	...
	if chosen_skill != "":
		match chosen_skill:
			"quick_strike":
				...
				action_log.append_array(result.log)
			...
			"whiplash":
				...
				if won:
					...
					action_log.append("[Whiplash] Opponent lost 1 Mana → now %d Mana" % opponent_mana)
				else:
					player_mana = max(0, player_mana - 1)
					action_log.append("[Whiplash] You lost 1 Mana → now %d Mana" % player_mana)
				action_log.append_array(result.log)
			...
			"soulbreak":
				...
				if won:
					...
					action_log.append("[Soulbreak] Stole %d Mana! You: %d | Opp: %d" % [stolen, player_mana, opponent_mana])
				action_log.append_array(result.log)
			...
			"rupture":
				...
				if won:
					...
					action_log.append("[Rupture] Opponent's next round base -20%")
				else:
					player_next_round_base_modifier = 0.8
					action_log.append("[Rupture] Your next round base -20%")
				action_log.append_array(result.log)
	...
	if won and not opponent_mark.is_empty():
		...
		action_log.append("[Mark] +%.1f flat damage from mark" % mark_dmg)

	if not player_mark.is_empty():
		opp_damage += player_mark.get("value", 2.0)
		action_log.append("[Mark] Opponent's mark deals %.1f to you" % opp_damage)
	...
	action_log.append("Round result | Player dmg: %.1f | Self HP delta: %.1f | Opp HP delta: %.1f"
...
func _quick_strike(wpm_mod: float, typo_penalty: float, won: bool, base_mult: float, char_base: float) -> Dictionary:
	...
	if won:
		if player_win_streak >= 1:
			damage *= 1.2
			action_log.append("[Quick Strike] Win Streak! ×1.20 → %.1f" % damage)
		else:
			damage *= 1.1
			action_log.append("[Quick Strike] Win ×1.10 → %.1f" % damage)
	else:
		if opponent_win_streak >= 1:
			damage = 0.0
			action_log.append("[Quick Strike] Loss on streak → 0 dmg")
		else:
			damage *= 0.9
			action_log.append("[Quick Strike] Loss ×0.90 → %.1f" % damage)

	return { "damage": damage, "log": action_log }
...
func _drain_touch(wpm_mod: float, typo_penalty: float, won: bool,
	...
	if won:
		...
		action_log.append("[Drain Touch] Win: Opponent bleeds %.1fHP/round × 3, you heal 2HP/round × 3" % bleed_val)
	else:
		player_bleed = { "rounds": 3, "value": 2.0 }
		action_log.append("[Drain Touch] Loss: You bleed 2HP/round × 3")

	return { "damage": damage, "player_hp_delta": player_hp_delta,
			"opp_hp_delta": opp_hp_delta, "log": action_log }
...
func _whiplash(acc_mod: float, typo_penalty: float, won: bool, base_mult: float, char_base: float) -> Dictionary:
	...
	if won:
		if opponent_win_streak >= 1:
			damage *= 2.0
			action_log.append("[Whiplash] Streak punish! ×2.0 → %.1f" % damage)
		else:
			damage *= 1.15
			action_log.append("[Whiplash] Win ×1.15 → %.1f" % damage)
	else:
		damage *= 0.85
		action_log.append("[Whiplash] Loss ×0.85 → %.1f" % damage)
...
func _soulbreak(wpm_mod: float, typo_penalty: float, won: bool, base_mult: float, char_base: float) -> Dictionary:
	...
	if won and player_mana >= 8:
		damage *= 1.15
		action_log.append("[Soulbreak] 8+ Mana bonus ×1.15 → %.1f" % damage)
	elif won:
		action_log.append("[Soulbreak] Win — base %.1f" % damage)
	else:
		action_log.append("[Soulbreak] Loss — no modifier")

	return { "damage": damage, "log": action_log }


func _rupture(opp_typos: int, typo_penalty: float, _won: bool, base_mult: float, char_base: float) -> Dictionary:
	var action_log: Array[String] = []
	var damage: float = 0.0

	if opp_typos == 0:
		action_log.append("[Rupture] Opponent had 0 typos → 0 damage (high risk!)")
		return { "damage": 0.0, "log": action_log }

	damage = (char_base + opp_typos * 1.5 - typo_penalty) * base_mult
	action_log.append("[Rupture] %.1f + (%d×1.5) - %.1f = %.1f" % [char_base, opp_typos, typo_penalty, damage])
...
func _deathmark(typo_penalty: float, won: bool, base_mult: float, opp_hp: float, char_base: float) -> Dictionary:
	...
	if won:
		...
		action_log.append("[Deathmark] Opponent marked: +%.1f flat/round × 2" % mark_val)
	else:
		player_mark = { "rounds": 2, "value": 2.0 }
		action_log.append("[Deathmark] You are marked: +2 flat/round × 2")

	return { "damage": damage, "opp_hp_delta": opp_hp_delta, "log": action_log }
...
func _tick_status_effects(player_hp: float, _opp_hp: float) -> Dictionary:
	var action_log: Array[String] = []
	var player_hp_delta: float = 0.0
	var opp_hp_delta:    float = 0.0
	...
	if not player_bleed.is_empty():
		...
		action_log.append("[Bleed] You take %.1f bleed damage (%d rounds left)" % [dmg, player_bleed["rounds"]])
	...
	if not opponent_bleed.is_empty():
		...
		action_log.append("[Bleed] Opponent takes %.1f bleed damage (%d rounds left)" % [dmg, opponent_bleed["rounds"]])
	...
	if not player_heal.is_empty():
		...
		action_log.append("[Heal] You recover %.1f HP (%d rounds left)" % [actual_heal, player_heal["rounds"]])
		if player_heal["rounds"] <= 0:
			player_heal = {}

	# Marks tick in resolve_round (applied after damage)

	return { "player_hp_delta": player_hp_delta, "opp_hp_delta": opp_hp_delta, "log": action_log }
